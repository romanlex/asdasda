import { Domain, Event, Store, combine, is, createDomain } from 'effector'
import { MatchedRoute } from 'react-router-config'

export const appDomain = createDomain('app')

const HATCH = 'app/page-hatch'
 ...
 */
export function createHatch(config_: Config | Domain = appDomain): Hatch {
  let domain
  ...
  const $opened = domain.createStore(Boolean(false), { sid: 'hatch.opened' })
  const $params = domain.createStore<Record<string, string>>({}, { sid: 'hatch.params' })
  const $query = domain.createStore<Record<string, string>>({}, { sid: 'hatch.query' })
